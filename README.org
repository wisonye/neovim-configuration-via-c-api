...
** The custom plugins I wrote include the following:

*** =Dired=: An Emacs major mode for directory browsing and editing, rewritten in Lua/rust to use in Neovim.

Example =dired buffer= in Neovim by pressing default binding: ~<c-c>j~
...
Example =Project command picker= in Neovim by pressing default binding: ~<leader>pc~
...
Example =Project command buffer= output when pressing ~<CR>~ on the selected command:
...
#+BEGIN_SRC bash
  ...
  └── src
      ...
      ├── keybindings.rs
      ├── utils.rs
      ├── my_dired.rs         # `dired` plugin
      ├── picker              # Common picker implementation
      │   ├── keybindings.rs
      │   ├── picker.rs
      │   └── popup_window.rs  
      ├── picker.rs
      └── project_command.rs  # `ProjectCommand` plugin
#+END_SRC
